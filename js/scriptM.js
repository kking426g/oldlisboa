                        ...
                        .append(
                            $("<div class='turnR'>").click(function() {
                                if ((icount + ii) > 0) {
                                    ...
                                    row.css({ 'margin-left': (192 * ii) - 5 + 'px' })
                                };
                                setTimeout(function() {
                                    row.css({ 'margin-left': 192 * ii + 'px' })
                                }, 100);
                            })
                        )
                        .append(
...
var red_border_style = function(elements, remove_attr) {
    ...
    elements.forEach(function(element) {
        if (remove_attr === false) {
            element.css({'background-color': '#ffcccc', 'border': 'solid #f00 2px'});
        }
        else {
...
var version = "v" + "20170126",
    ...
    marquee = function() {
        ...
        ///init
        if (dom.length < 0 || link.length < 0) { return };
    ...
    jInfo = function(str, til, sec) {
        sec = sec || 600;
        if ($("body.lock, body.openmenu, body.openmmenu").length >= 1) { return };
        //文字檢查
        ...
        //有title的话
        if (typeof til == "string" && til.length >= 0) { $("<h2>").text(til).prependTo(".mbox"); };
        //自动关闭
        setTimeout(function() {
            if ($("<div class=' " + rn_id + "'>").length == 1) {
                init();
    ...
    */
    jAlert = function(str, til) {
        ...
        $("<div class='mbox'>").append(
            ...
            ).append($("<button class='sure' type='button'>").on("click", function() { init(); }))
            .appendTo("div.mask");
        //有title的话
        if (typeof til == "string" && til.length >= 0) { $("<h2>").text(til).prependTo(".mbox"); };
    },
    ...
    */
    jConfirm = function(str, callback) {
        ...
        $("<div class='mbox'>")
            ...
            .append($("<button class='default done' type='button'>"+js_lang.determine+"</button>").on("click", function() {
                init();
                (callback) ? callback(true): '';
            }))
            .append($("<button class='default cancel' type='button'>"+js_lang.cancel+"</button>").on("click", function() {
                init();
            ...
            data.forEach(function(D) {
                ...
                if (D.performType.performTypeId == 2) {
                    ...
                    if (game_data.info.hot) {
                        tag.append($("<span class='hot'>"+js_lang.hot+"</sapn>"))
                    }
                    ;
                    //精選
                    if (game_data.info.choice) {
                        tag.append($("<span class='choice'>"+js_lang.featured_sim+"</sapn>"))
                    }
                    ;
                    //新品
                    if (game_data.info.newest) {
                        tag.append($("<span class='newest'>"+js_lang.new_sim+"</sapn>"))
                    }
                    ;
                    //寫入data
                    ...
                    //寫入data
                        ...
                        .append(
                            $("<div class='pitem'>").append(
                                ...
                                .append(
                                    //遊戲名稱
                                    $("<p>", {text: game_data.displayName || game_data.gameName})
                                )
                    ...
                    //寫入data
                        ...
                        .append(
                            $("<div class='pitem'>")
                                .append(
                                    //遊戲名稱
                                    $("<p>", {class: 'title', text: D.displayName || D.gameName})
                                )
                                .append(
                                    //圖片
                                    $("<div class='pbox'>").append(
    ...
    game = {
            ...
            if (islogin() && ocmsite) {
                ...
                ocmsite.game.loadGameById({
                    ...
                    onDone: function() {
                        ...
                        if(ocmsite.game.setCurrentGameById("1524") != null) {
                            ...
                            $("<div class='card'>")
                                ...
                                .append(
                                    $("<div class='inputgroup trans hall'>").append(
                                        $("<span>").text(me.gamePlatform.name)
                                    ).append(
                                ...
                                //转出金额
                                .append($("<button class='trans out' type='button'>").on("click", function () {
                                ...
                                //开始游戏
                                .append($("<button class='trans play' type='button'>").on("click", function () {
                                ...
                                //
                                .append($("<button class='trans return' type='button'>").on("click", function () {
                                    init();
                                }))
                                .appendTo("div.mask");
        ...
        exchange: function(me) {
            ...
            $("<div class='card'>")
                ...
                //转出金额
                .append($("<button class='default trans out' type='button'>"+js_lang.trans_out_money+"</button>").on("click", function() {
                    var amount = parseInt($("input.trans.money").val());
                    var onDone = function() {
                        //
                        $(".trans.account").val(parseInt($(".sys_txt_balance").text()));
                ...
                //开始游戏
                .append($("<button class='trans play default' type='button'>"+js_lang.enter_game_room+"</button>").on("click", function() {
                    game.play(me);
                }))
                //
                .append($("<button class='trans return default' type='button'>"+js_lang.return+"</button>").on("click", function() {
                    init();
                }))
                .appendTo("div.mask");
        },
            ...
            if (islogin()) {
                ...
                if(me !== null) {
                    ...
                    $("<div class='card'>")
                        ...
                        //存款
                        .append($("<button class='go _withdrawal default' type='button'>"+js_lang.deposit+"</button>").on("click", function () {
                            game.exchange(me);
                        }))
                        //開始遊戲
                        .append($("<button class='go _start default' type='button'>"+js_lang.play_win+"</button>").on("click", function () {
                            game.play(me);
                        }))
                        //收藏鍵
                        //顯示文字：false 加入收藏; true 取消收藏;
                        .append($("<button class='default fav' type='button'>").addClass(me.is_fav.toString()).text(me.is_fav ? js_lang.remove_favorites : js_lang.add_favorites).on("click", function () {
                            game.fav(me);
                            //先不管了 要接回應呀
                ...
                $("<div class='card'>")
                    .append($("<img>", { src: me.game_img }))
                    .append($("<h1>").text(me.displayName || me.gameName))
                    .append($("<button class='go _login' type='button'>").on("click", function() {
                        login();
                    }))
                    .append($("<button class='go _signup' type='button'>").on("click", function() {
        ...
        b2b: function() {
            ...
            if (tran_Out_amount <= 0) {
                ...
                ocmsite.user.sendTransferPlatformToPlatforme(
                    ...
                    function(data) { jAlert(data.errorMessage, js_lang.failed_operation) }, // 操作失败
                    function(data) {
                        // if (typeof data == "object") {
...
$(function() {
    ...
    $("button.menu, a.close_menu").click(function() {
        ...
        $("header").stop(!1, !1).toggleClass('open', 2000);
    });
    ...
    $("button.member, div.info").click(function() {
        ...
        $("article.lightbox").hide();
        if (islogin()) {
            $(".menu_right").stop(!1, !1).toggleClass('openmenu');
            $("header").stop(!1, !1).toggleClass('open', 2000);
        }
    });
